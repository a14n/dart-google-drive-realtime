...
Future<Document> load(String docId, [void initializerFn(Model model), void errorFn(Error error)]) {
  ...
  realtime.load(docId,
      (js.Proxy p) => completer.complete(Document.cast(p)),
      initializerFn == null ? null : (js.Proxy p) => initializerFn(Model.cast(p)),
      errorFn == null ? null : (js.Proxy p) => errorFn(Error.cast(p))
  );
